...
open class Spatial : Node() {
    ...
    fun moveAndSlide(boundingBox: BoundingBox, velocity: Vec3f) {
        val v = velocity.copy()
        val a = v.normalized().apply { selfMultiply(0.1f) }
        while (v.length > 0.1f) {
            v.selfSubtract(a)
    ...
    fun isOnSurface(boundingBox: BoundingBox, normal: Vec3f = Vec3f.UP, maxDistanceFromSurface: Float = 0.1f): Boolean {
        val r = normal * maxDistanceFromSurface
