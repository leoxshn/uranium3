    ...
    fun update() {
        try {
            ...
            s[2] = if (posInChunk.x == Chunk.SIZE - 1)
                chunk.chunkMap[chunkPos.copy(x = chunkPos.x + 1)]?.get(posInChunk.copy(x = 0)) == null
            else chunk[posInChunk.x + 1, posInChunk.y, posInChunk.z] == null
            s[3] = if (posInChunk.x == 0)
                chunk.chunkMap[chunkPos.copy(x = chunkPos.x - 1)]?.get(posInChunk.copy(x = Chunk.SIZE - 1)) == null
            else chunk[posInChunk.x - 1, posInChunk.y, posInChunk.z] == null
            s[1] = if (posInChunk.y == Chunk.SIZE - 1)
                chunk.chunkMap[chunkPos.copy(y = chunkPos.y + 1)]?.get(posInChunk.copy(y = 0)) == null
            else chunk[posInChunk.x, posInChunk.y + 1, posInChunk.z] == null
            s[4] = if (posInChunk.y == 0)
                chunk.chunkMap[chunkPos.copy(y = chunkPos.y - 1)]?.get(posInChunk.copy(y = Chunk.SIZE - 1)) == null
            else chunk[posInChunk.x, posInChunk.y - 1, posInChunk.z] == null
            s[0] = if (posInChunk.z == Chunk.SIZE - 1)
                chunk.chunkMap[chunkPos.copy(z = chunkPos.z + 1)]?.get(posInChunk.copy(z = 0)) == null
            else chunk[posInChunk.x, posInChunk.y, posInChunk.z + 1] == null
            s[5] = if (posInChunk.z == 0)
                chunk.chunkMap[chunkPos.copy(z = chunkPos.z - 1)]?.get(posInChunk.copy(z = Chunk.SIZE - 1)) == null
            else chunk[posInChunk.x, posInChunk.y, posInChunk.z - 1] == null
            sides = s
        } catch (e: Exception) {
            e.printStackTrace()
